...
from competitions.api.v1.serializers import (
    ...
    ClassificationSerializer, CompetitionTeamsInfoSerializer, CompetitionTeamRequestSerializer, MatchFinishRequestSerializer
)
...
class CompetitionsAPIView(APIView):
    """
    View para listar e criar Competições.
...
class CompetitionRetrieveUpdateDestroyAPIView(APIView):
    """
    View para ver, atualizar e deletar uma Competição específica.
    """
...
class CompetitionSetInProgress(APIView):
    """
    Altera o status de uma competição para "em andamento".
    """
...
class CompetitionSetFinished(APIView):
    """
    Altera o status de uma competição para "finalizada".
    """
    permission_classes = [IsAuthenticated]
...
class CompetitionTeamsAPIView(APIView):
    """
    View para listar e gerenciar os times inscritos em uma competição.
    """
    ...
    @extend_schema(
        ...
        request=CompetitionTeamRequestSerializer,
        responses={200: OpenApiResponse(description="O time pode ser inscrito."), 409: OpenApiResponse(
    ...
    def post(self, request, competition_id):
        ...
        if not team_id_from_request:
            return Response({"message": "O campo 'team_id' é obrigatório."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if has_role(groups, "Organizador", "Jogador"):
            ...
            if team_exists:
                return Response({"can_be_inscribed": False, "message": "A equipe já está inscrita nesta competição."}, status=status.HTTP_409_CONFLICT)
            else:
                serializer = CompetitionTeamsInfoSerializer(competition)
                return Response({"can_be_inscribed": True, "message": "A equipe pode ser inscrita nesta competição.", "data": serializer.data}, status=status.HTTP_200_OK)
        else:
            raise PermissionDenied(
...
class GenerateCompetitionsAPIView(APIView):
    """
    Gera as fases (grupos, eliminatórias) e partidas de uma competição.
    """
    permission_classes = [IsAuthenticated]
    ...
    @extend_schema(
        ...
        description="Este endpoint aciona a lógica de criação de grupos, rodadas e partidas com base no sistema da competição (pontos corridos, eliminatórias, etc.).",
        request=None,
...
class EndGroupStageAPIView(APIView):
    """
    Para competições de 'grupos e eliminatórias', finaliza a fase de grupos e gera a fase eliminatória.
    """
    permission_classes = [IsAuthenticated]
    ...
    @extend_schema(
        ...
        description="Calcula os classificados da fase de grupos e gera as partidas da fase eliminatória (oitavas, quartas, etc.).",
        request=None,
        responses={200: OpenApiResponse(
            description="Fase eliminatória gerada com sucesso.")}
    )
    def post(self, request, competition_id):
        """
        ...
        if has_role(groups, "Organizador"):
            if competition.system == 'groups_elimination':
                ...
                return Response({"error": "This endpoint is only for competitions with 'groups_elimination' system."}, status=status.HTTP_400_BAD_REQUEST)
        else:
            raise PermissionDenied(
...
class CompetitionTeamRetrieveUpdateDestroyAPIView(APIView):
    ...
    @extend_schema(
        ...
        responses={200: CompetitionTeamSerializer}
    )
    ...
    @extend_schema(
        tags=["Times em Competição"],
        summary="Atualiza a inscrição de um time (reservado para uso futuro)",
    ...
    def get(self, request, competition_id):
        ...
        rounds_queryset = Round.objects.filter(
            ...
            Prefetch('match_set', queryset=Match.objects.select_related(
                'team_home', 'team_away', 'competition', 'group', 'round'))
        ).distinct()
        paginator = PageNumberPagination()
...
class RoundMatchesAPIView(APIView):
    ...
    def get(self, request, round_id):
        ...
        round = Round.objects.filter(id=round_id).prefetch_related(
            Prefetch('match_set', queryset=Match.objects.select_related(
                'team_home', 'team_away', 'group', 'round', 'competition'))
        ).distinct().first()
...
class MatchesAPIView(APIView):
    """
    View global para listar partidas de todas as competições de um campus.
    """
    permission_classes = [AllowAny]
    ...
    def get(self, request):
        ...
        if auth_header and auth_header.startswith("Bearer "):
            ...
            try:
                ...
                raise AuthenticationFailed("Token inválido ou expirado.")
        if not campus_code:
            return Response({"detail": "Campus não especificado."}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def get(self, request, competition_id=None):
        ...
        if campus_code:
            matches_queryset = Match.objects.filter(
                competition__modality__campus=campus_code, scheduled_datetime__date=today)
        else:
            competition = get_object_or_404(Competition, id=competition_id)
            matches_queryset = Match.objects.filter(
                competition=competition, scheduled_datetime__date=today)
...
class MatchFinishAPIView(APIView):
    ...
    @extend_schema(
        ...
        request=MatchFinishRequestSerializer,
        responses={200: OpenApiResponse(
    ...
    def patch(self, request,  match_id):
        ...
        if has_role(groups, "Organizador"):
            if match.status == 'in-progress':
                ...
                # Usando o serializer para validar os dados de entrada
                serializer = MatchFinishRequestSerializer(data=request.data)
                serializer.is_valid(raise_exception=True)
                score_home = serializer.validated_data['score_home']
                score_away = serializer.validated_data['score_away']

                # Lógica de negócio agora usa dados validados
                finish_match(match, score_home, score_away)
                new_data = MatchSerializer(match).data
                log_payload = generate_log_payload(
                    event_type="match.updated",
                    service_origin="competitions_service",
